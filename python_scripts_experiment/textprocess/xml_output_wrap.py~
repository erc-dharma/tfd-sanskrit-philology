...
def xml_output(filename, apparatus_type):
    ...
    for line in openfile:
        if '<START/>' in line:
            onflag = True
        if '<STOP/>' in line:
            print("</apparatuswrap></mainwrap></body>")        
            quit()
        ...
        if '<NEWCHAPTER/>' in line:
            ...
            if firstTEXT == False:
               print('</apparatuswrap></mainwrap><NEWCHAPTER/>')
            else:
               print('<NEWCHAPTER/>')
            firstTEXT = True
        if '<SETVSNUM' in line:
        ...
        if '<TEXT>' in line and firstTEXT == False:
             line = re.sub('<TEXT>', '</apparatuswrap></mainwrap><mainwrap><TEXT>', line)
        if '<TEXT>' in line and firstTEXT == True:
             line = re.sub('<TEXT>', '<mainwrap><TEXT>', line) 
             firstTEXT = False
